...
import { allPages as allMockPages, frequentPages as frequentMockPages } from './mocks';
...
const App = () => {
  const [frequentPages, setFrequentPages] = useState<PageConfig[]>(frequentMockPages);
  const [allPages, setAllPages] = useState<Array<PageConfig | FolderConfig>>(allMockPages);
  ...
  const handleDragEndFrequentPages = (event: { active: any; over: any; }) => {
    ...
    if (active.id !== over.id) {
      setFrequentPages((items) => {
        const oldIndex = items.findIndex(({ sortIndex }) => sortIndex === active.id);
        const newIndex = items.findIndex(({ sortIndex }) => sortIndex === over.id);
